			...
			PTP_DevicePropDescTypdef* devicepropertydesc)
{
  ...
  case PTP_REQ_WAIT:
    ...
    if(status == USBH_OK)
    {
			...
			devicepropertydesc->DataType = LE16(&data[PTP_dpd_DataType]);
			devicepropertydesc->GetSet = *(uint8_t *)(&data[PTP_dpd_GetSet]);
			devicepropertydesc->FactoryDefaultValue.u8 = *(uint8_t *)(&data[PTP_dpd_FactoryDefaultValue]);
			devicepropertydesc->CurrentValue.u8 = *(uint8_t *)(&data[PTP_dpd_FactoryDefaultValue+1]);
			devicepropertydesc->FormFlag = *(uint8_t *)(&data[7]);
		...
		/*	//Variable-length Unicode String range����
			...
			devicepropertydesc->CurrentValue.str[strlength]='\0';
			devicepropertydesc->FormFlag = *(int8_t *)(DevicePropertyPtr);
			if(devicepropertydesc->FormFlag==0x02)
			{
				DevicePropertyPtr+=1;
				devicepropertydesc->FORM.Enum.NumberOfValues = *(uint16_t *)(DevicePropertyPtr);
				DevicePropertyPtr+=2;
